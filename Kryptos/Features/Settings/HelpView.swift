...
private let faqItems: [HowToStep] = [
    ...
    HowToStep(title: "Is the post-quantum protection on every message?",
              text: "Yes. Kryptos is built on libsignal, where the SPQR post-quantum ratchet has been permanently on since version 0.83.0: the key of every message is derived with post-quantum material, not just the handshake. There is no switch for it, in Kryptos or in the library itself."),
