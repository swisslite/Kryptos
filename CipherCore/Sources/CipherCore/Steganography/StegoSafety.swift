...
public enum StegoSafety {
    public static let runtimeMinLength = 4
    public static let hanRuntimeMinLength = 2

    public static let seedAttempts = 256
    static let minSeedAttempts = 8
    static let seedBudgetBytes = 500_000

    public static func seedAllowance(for payload: Int) -> Int {
        let byLength = seedBudgetBytes / max(payload, 1)
        return min(seedAttempts, max(minSeedAttempts, byLength))
    }

    static func firstCleanCover(payload: Int, _ make: (Int) -> String) -> String? {
        let start = Int(randomBytes(1)[0])
        for attempt in 0 ..< seedAllowance(for: payload) {
            let text = make((start + attempt) & 0xFF)
    ...
    private static let exactSet: Set<String> = Set(exactWords)
    private static let prefixTable = Table(stems + hanStems)
    private static let runtimeTable = Table(stems.filter { $0.count >= runtimeMinLength }
        + hanStems.filter { $0.count >= hanRuntimeMinLength })

    public static func blocks(_ word: String) -> Bool {
        var folded = word.lowercased()
        if exactSet.contains(folded) { return true }
        return folded.withUTF8 { prefixTable.matchesAt(0, $0) }
    }

    public static func containsBlocked(_ text: String) -> Bool {
        var folded = text.lowercased()
        return folded.withUTF8 { runtimeTable.scan($0) }
    }
    ...
    private struct Table {
        ...
        init(_ words: [String]) {
            let encoded = words.map { Array($0.utf8) }
            var slots = [[Int]](repeating: [], count: 256)
            for (i, stem) in encoded.enumerated() {
                guard let first = stem.first else { continue }
                let slot = first < 0x80 ? Int(first) : Int(stem.count > 1 ? stem[1] : first)
                slots[slot].append(i)
            }
            var blob = [UInt8]()
            var starts = [Int32](repeating: 0, count: 257)
            var pairs = [Int32]()
            var placed = 0
            for slot in 0 ..< 256 {
                starts[slot] = Int32(placed)
                for i in slots[slot] {
                    pairs.append(Int32(blob.count))
                    pairs.append(Int32(encoded[i].count))
                    blob.append(contentsOf: encoded[i])
                    placed += 1
                }
            }
            starts[256] = Int32(placed)
            bytes = blob
            index = starts + pairs
            shortest = encoded.map(\.count).min() ?? 1
        }

        private static let bucketCount = 257

        func matchesAt(_ at: Int, _ buffer: UnsafeBufferPointer<UInt8>) -> Bool {
            bytes.withUnsafeBufferPointer { stemBytes in
                index.withUnsafeBufferPointer { table in
                    hit(at, buffer, stemBytes, table)
                }
            }
        }

        func scan(_ buffer: UnsafeBufferPointer<UInt8>) -> Bool {
            guard buffer.count >= shortest else { return false }
            let last = buffer.count - shortest
            return bytes.withUnsafeBufferPointer { stemBytes in
                index.withUnsafeBufferPointer { table in
                    var at = 0
                    while at <= last {
                        if hit(at, buffer, stemBytes, table) { return true }
                        at += 1
                    }
                    return false
                }
        ...
        private func hit(
            ...
            _ stemBytes: UnsafeBufferPointer<UInt8>,
            _ table: UnsafeBufferPointer<Int32>,
        ) -> Bool {
            let byte = buffer[at]
            let slot: Int
            if byte < 0x80 {
                slot = Int(byte)
            } else if byte < 0xC0 || at + 1 >= buffer.count {
                return false
            } else {
                slot = Int(buffer[at + 1])
            }
            var k = Int(table[slot])
            let end = Int(table[slot + 1])
            while k < end {
                let offset = Int(table[Table.bucketCount + 2 * k])
                let length = Int(table[Table.bucketCount + 2 * k + 1])
                if at + length <= buffer.count {
                    var j = 0
                    while j < length, buffer[at + j] == stemBytes[offset + j] { j += 1 }
